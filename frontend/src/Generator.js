...
//     try {
//       // 调用后端 API 获取 3 组数字
//       const data = await fetchGeneratedNumbers(3); // 每次请求 3 组数字
//       setResults(data);
//     } catch (err) {
...
//           <div key={index} className="number-display-row">
...
//       <button
//         className={`generate-button ${loading || isDisplaying ? "disabled" : ""}`}
...
// 下述代码可以运行mega million generator
...
function Generator() {
  ...
  const generateNumbers = async () => {
    ...
    try {
      const data = await fetchGeneratedNumbers("mega_millions", 3);
    ...
    <div className="generator-container">
      ...
      <div className="number-display-wrapper">
        {displayedResults.map((result, index) => (
          <div key={index} className="number-display-row fade-in">
            {result.numbers.map((num, idx) => (
              <div key={idx} className="number filled">{num}</div>
            ))}
            <div className="mega-ball filled">{result.mega_ball}</div>
          </div>
        ))}
