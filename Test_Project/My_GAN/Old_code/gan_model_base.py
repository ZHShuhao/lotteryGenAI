...
#
# # 数据增强：加入轻微扰动
# data_tensor += 0.01 * torch.randn_like(data_tensor)
#
# # 条件信息
...
#         self.fc3 = nn.Linear(1024, number_dim + mega_dim)
#         self.activation = nn.ReLU()
#         self.bn1 = nn.BatchNorm1d(512)
#         self.bn2 = nn.BatchNorm1d(1024)
#
#         # 调整 x_residual 的尺寸
#         self.residual_adjust = nn.Linear(512, 1024)
#
#     def forward(self, noise, condition):
#         x = torch.cat([noise, condition], dim=1)
#         x = self.activation(self.bn1(self.fc1(x)))
#         x_residual = self.residual_adjust(x)  # 调整尺寸
#         x = self.activation(self.bn2(self.fc2(x))) + x_residual  # 跳跃连接
#         x = self.fc3(x)
#         numbers = torch.softmax(x[:, :number_dim].view(-1, 5, num_classes), dim=-1)
#         mega = torch.softmax(x[:, number_dim:].view(-1, mega_classes), dim=-1)
#         return numbers, mega
#
#
...
#         noise = torch.randn(len(real_batch), latent_dim).to(device)
#         numbers, mega = G(noise, condition_batch)
#         generated_data = torch.cat([numbers.view(numbers.size(0), -1), mega], dim=1)
#         g_loss = -D(generated_data[:, :number_dim], generated_data[:, number_dim:], condition_batch).mean()
#
...
# generated_numbers = numbers.argmax(dim=-1).cpu().numpy() + 1
# generated_mega = mega.argmax(dim=-1).cpu().numpy() + 1
#
# # 打印生成结果
# for i in range(100):
...
'''
上述代码可以很好生成数字。只不过还有一些些许的偏差。同时存在Number1-Number5 重复数字的情况
以下代码融入了加权随机抽样， 确保Number1-Number5 不会有重复的数字
'''
