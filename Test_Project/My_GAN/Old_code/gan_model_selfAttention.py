...
# 数据增强：加入轻微扰动
data_tensor += 0.02 * torch.randn_like(data_tensor)
...
#         x_residual = self.residual_adjust(x)
#         x = self.activation(self.bn2(self.fc2(x))) + x_residual
#         x = self.fc3(x)
#
...
#         self.fc1 = nn.Linear(latent_dim + condition_dim, 512)
#         self.fc2 = nn.Linear(512, 1024)
#         self.self_attention = nn.MultiheadAttention(embed_dim=1024, num_heads=16, batch_first=True)
#         self.fc3 = nn.Linear(1024, number_dim + mega_dim)
#         self.activation = nn.ReLU()
#         self.bn1 = nn.BatchNorm1d(512)
#         self.bn2 = nn.BatchNorm1d(1024)
